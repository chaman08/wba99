...
export const PhysioActivity = () => {
    ...
    // Scoped to the current physiotherapist
    const myActivities = useMemo(() => {
        return [...cases]
            .filter((c) => c.physiotherapistId === authUser?.id)
    ...
    const statusMap: Record<string, { label: string; class: string; icon: any }> = {
        ...
        "In Review": { label: "In Review", class: "bg-primary/10 text-primary", icon: <FileText className="w-3 h-3" /> },
        "Report Ready": { label: "Report Ready", class: "bg-success/10 text-success", icon: <CheckCircle className="w-3 h-3" /> },
        Completed: { label: "Completed", class: "bg-white/5 text-text-muted", icon: <CheckCircle className="w-3 h-3" /> },
    };
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-12">
            <header className="rounded-3xl bg-surface/70 p-6 shadow-soft-light border border-white/5">
                ...
                <p className="text-sm text-text-muted mt-1">Track all your submitted cases and report statuses.</p>
            </header>
            ...
            <div className="grid gap-4">
                {myActivities.length === 0 ? (
                    <div className="rounded-3xl bg-surface/70 p-12 text-center border border-white/5 border-dashed">
                        ...
                        <Link to="/cases/new" className="mt-4 inline-block text-primary hover:underline text-sm font-semibold">
                    ...
                    myActivities.map((item) => (
                        <article
                            key={item.id}
                            onClick={() => navigate(item.status === 'Report Ready' ? `/cases/${item.id}/report` : `/cases/${item.id}`)}
                            ...
                            <div className="flex items-start gap-4 flex-1">
                                ...
                                <div className="space-y-1">
                                    <div className="flex items-center gap-3">
                                        <span className="text-[10px] uppercase tracking-widest text-text-muted font-bold">{item.id.split('-')[1]}</span>
                                        <span className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${statusMap[item.status]?.class}`}>
                                    ...
                                    <p className="text-xs text-text-muted flex items-center gap-1.5">
                                        ...
                                        Last activity: {new Date(item.updatedAt).toLocaleDateString()} at {new Date(item.updatedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                    </p>
                                </div>
                            </div>

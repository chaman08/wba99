...
export const AdminCases = () => {
  ...
  const filtered = useMemo(() => {
    return allCases.filter((item) => {
      ...
      const matchesExpert = expertFilter === "All" || item.expertId === expertFilter;
      const matchesQuery = item.title.toLowerCase().includes(query.toLowerCase());
      return matchesStatus && matchesPhysio && matchesExpert && matchesQuery;
    });
  }, [allCases, statusFilter, physioFilter, expertFilter, query]);
    ...
    <section className="space-y-6 rounded-3xl bg-surface/70 p-6 shadow-soft-light animate-fade-in">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <select value={expertFilter} onChange={(event) => setExpertFilter(event.target.value)} className="rounded-2xl border border-white/10 bg-background px-4 py-2 text-sm text-text">
            <option value="All">Expert</option>
            {experts.map((expert) => (

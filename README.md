...
- **Role-based SaaS shell** – physiotherapist, expert reviewer, and admin routes are guarded by Zustand-powered auth backed by Firebase Firestore persistence.
...
- **Firebase bootstrap** lives in `src/lib/firebase.ts` and the store now reads/writes identities, patients, cases, and reports through Firestore.
